...
func doMap(
	...
	//
	/*
	   mapF函数可以从infile获取中间集合[]KeyValue
	   我们需要做的是将中间集合的每一个值按照规律写入文件中。
	   规律是每一个值的hash确定哪一个文件。
	   文件的命名规则是根据mapnum,reducenum决定的，此处可以直接通过reduceName函数获取.
	   读取一个文件产生nReduce文件
	*/
	...
	for i := 0; i < nReduce; i++ {
		files[i], _ = os.Create(reduceName(jobName, mapTaskNumber, i))
		//files[i]
		enc[i] = json.NewEncoder(files[i])
	}
	for _, kvvalue := range kvs {
		hashNum := ihash(kvvalue.Key)
		hashNum = hashNum % nReduce
		enc[hashNum].Encode(kvvalue)
	}
